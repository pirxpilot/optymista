...
function optymista(processArgs = process.argv.slice(2)) {
  ...
  let _strict = true;
  let _allowNegative = true;
  let _allowPositionals = true;
  ...
  function version(v = true) {
    _version = v;
    return self.boolean('version').short('V').describe('show version and exit');
  }
  ...
  function strict(s = true) {
    _strict = s;
    return self;
  }

  function allowNegative(n = true) {
    _allowNegative = n;
    return self;
  }

  function allowPositionals(n = true) {
    _allowPositionals = n;
    return self;
  }
  ...
  function parseArgs(args) {
    try {
      const { values, positionals } = util.parseArgs({
        args,
        allowPositionals: _allowPositionals,
