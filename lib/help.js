...
function help(usage, options = {}) {
  const keys = Object.keys(options).filter(key => key !== 'help');
  ...
  const switches = Object.fromEntries(keys.map(key => makeSwitch(key, options[key])));
  ...
  keys.forEach(key => {
    ...
    const body = [
      ...
      'default' in options[key] ? `[default: ${JSON.stringify(options[key].default)}]` : null
    ]
      .filter(Boolean)
      .join(' ')
      .trimEnd();
...
function makeSwitch(key, { short, type }) {
  let str = [short, key].filter(Boolean).map(optionize).join(', ');
